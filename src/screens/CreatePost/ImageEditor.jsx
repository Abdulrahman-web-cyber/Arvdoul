...
const DARK = {
  ...
  glass: 'rgba(3,7,27,0.85)',
  canvasBg: 'radial-gradient(ellipse at center, #0B1130 0%, #03071B 70%)',
  checkerboard: '#111833',
...
const ImageEditor = forwardRef(({ media, onClose, onSave, additionalMedia = [] }, ref) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    setIsSaving(true);
    try {
      await handleExport('png', 1, doc.imageDimensions.width, doc.imageDimensions.height);
      onClose?.();
    } catch {
  ...
  }, [isSaving, handleExport, doc.imageDimensions, onClose]);
