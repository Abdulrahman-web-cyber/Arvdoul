...
export default function ContentEditor({
  ...
  const [aiHooksError, setAiHooksError] = useState(null);

  // Hooks are generated by the server-side AI gateway (aiStudioService). No
  // canned "viral" strings are shown: if the gateway is unavailable the panel
  // reports it instead of inventing engagement bait.
  const generateAiHooks = useCallback(async () => {
    setAiHooksLoading(true);
    setAiHooksError(null);
    try {
      const { default: aiStudioService } = await import('../../services/aiStudioService.js');
      const topic = (state.content || '').trim().slice(0, 280);
      ...
      if (!result) {
        setAiHooks([]);
        setAiHooksError('AI hooks are unavailable right now. Check that the AI gateway is configured.');
        return;
      }
      setAiHooks([result.hook, result.body].filter(Boolean));
    } catch {
      setAiHooks([]);
      setAiHooksError('Could not generate AI hooks. Please try again.');
    } finally {
  ...
  const handleToggleAiPrompts = useCallback(() => {
    setShowAiPrompts((open) => {
      const next = !open;
      if (next && aiHooks.length === 0 && !aiHooksLoading) generateAiHooks();
      return next;
    });
  }, [aiHooks.length, aiHooksLoading, generateAiHooks]);
      ...
      <div id="content-editor" ref={textareaContainerRef} className="relative" onKeyDown={handleMentionKeyDown}>
        ...
        <AnimatePresence>
          {showAiPrompts && (
            ...
            >
              <p className="text-[11px] font-bold text-purple-300 mb-2">AI-suggested hook:</p>
              {aiHooksLoading && (
                <div className="flex items-center gap-2 py-2 text-xs text-purple-200">
                  <LoadingSpinner size="sm" />
                  Generating hooks…
                </div>
              )}
              {!aiHooksLoading && aiHooksError && (
                <div className="flex items-center justify-between gap-3 py-1">
                  <p className="text-xs text-purple-200">{aiHooksError}</p>
                  <button
                    onClick={generateAiHooks}
                    className="shrink-0 rounded-lg border border-purple-500/30 px-2.5 py-1 text-xs font-semibold text-purple-200 hover:bg-purple-900/60"
                  >
                    Retry
                  </button>
                </div>
