...
export default function LoginScreen() {
  ...
  useEffect(() => {
    ...
    const initializeRecaptcha = async () => {
      try {
        ...
        console.warn("reCAPTCHA pre-init notice:", error?.message);
      }
    };
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      if (result?.success) {
        if (result.requiresVerification || result.user?.isUnverified || (result.user?.emailVerified === false && result.user?.authProvider === 'email')) {
          navigate("/verify-email", {
            replace: true,
            state: {
              email: result.user?.email || email,
              userId: result.user?.uid || result.user?.userId,
              fromLogin: true
            }
          });
          return;
  ...
  return (
    <div className={`h-[100dvh] flex flex-col ${resolvedTheme === 'dark' ? 'bg-[#03071B] text-white' : 'bg-[#F6F8FC] text-gray-900'}`}>
      ...
      {/* reCAPTCHA container */}
      <div id="login-recaptcha-container" className="fixed bottom-0 right-0 z-50 pointer-events-auto" />
    </div>
  );
}