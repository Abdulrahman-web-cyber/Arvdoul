...
export default function PostOptionsDrawer({ isOpen, onClose, post, currentUser, theme, onAnalytics }) {
  ...
  const ownerId = safePost.authorId || safePost.userId || safePost.ownerId;
  ...
  const [authorMeta, setAuthorMeta] = useState(() => {
    const cached = ownerId ? authorMetaCache.get(ownerId) : null;
    if (cached && Date.now() - cached.timestamp < CACHE_TTL) return cached.data;
    return {
      position: null,
      followerCount: safePost.followerCount || 0,
      isCreator: safePost.isCreator || false,
      isVerified: safePost.isVerified || false,
      creatorScore: null,
      loading: true,
    };
  });
  ...
  const [uiState, dispatchUI] = useReducer(uiReducer, {
    ...initialState,
    saved: safePost.savedBy?.includes(currentUser?.uid) || false,
    pinned: !!safePost.pinned,
    archived: safePost.archived === true,
    commentsEnabled: safePost.commentsEnabled !== false,
    followState: null,
  });
  ...
  const hasMedia = !!(safePost.media && safePost.media.length > 0);
  const mediaUrl = hasMedia ? safePost.media[0].url : null;
  const mediaType = safePost.type || (safePost.media?.[0]?.type);
  const postUrl = typeof window !== 'undefined' ? `${window.location.origin}/post/${safePost.id || ''}` : '';
