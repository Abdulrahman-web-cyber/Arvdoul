...
export default function AppStateGuard({ children }) {
  ...
  useEffect(() => {
    // If public route, render immediately even while auth initializes
    if (isPublicRoute) {
    ...
    if (isAuthenticated && (!needsEmailVerification || isEmailVerified) && isProfileComplete && isSetupProfileRoute) {
  ...
  // Subtle non-intrusive transition while auth resolves
  if (!shouldRender) {
    return (
