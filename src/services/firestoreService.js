...
class EnterpriseFirestoreService {
  ...
  async likePost(postId, userId) {
    ...
    try {
      const { doc, updateDoc, arrayUnion, increment, serverTimestamp } = this.firestoreMethods;
  ...
  async sendGift(postId, userId, giftType, coinValue) {
    ...
    try {
      const { collection, addDoc, doc, updateDoc, increment, arrayUnion, serverTimestamp, getDoc } = this.firestoreMethods;
