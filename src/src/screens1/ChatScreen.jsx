...
export default function ChatScreen() {
  ...
  const loadMessages = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setPagination({ lastDoc: result.nextCursor, hasMore: result.hasMore });
      }
    } catch (err) {
  ...
  const savePendingMessage = (message) => {
    try {
      ...
      if (pending) {
        ...
        parsed.forEach(async (msg) => {
          try {
            ...
            removePendingMessage(msg._clientId);
          } catch (err) {}
        });
      }
    };
  ...
  const sendMessage = async () => {
    ...
    try {
      if (!navigator.onLine) {
        await savePendingMessage({ _clientId: clientId, data: messageData });
  ...
  const addParticipants = () => {
    // In a real app, open a user picker modal
  ...
  const MessageBubble = ({ msg }) => {
    ...
    const handleReaction = (reaction) => {
      messagingService.reactToMessage(conversationId, msg.id, user.uid, reaction);
    };
        ...
        <div className={cn('max-w-[70%]', isOwn ? 'items-end' : 'items-start')}>
          {msg.replyTo && (
            <div className="bg-gray-100 dark:bg-gray-800 rounded-lg p-2 mb-1 text-sm">
              <span className="text-xs text-gray-600 dark:text-gray-400">Replying to {msg.replyTo.senderName}</span>
              <p className="text-gray-900 dark:text-white truncate">{msg.replyTo.contentPreview}</p>
            </div>
          )}
          ...
          >
            {/* Content by type */}
            {msg.type === MESSAGE_TYPES.TEXT && <p>{msg.content}</p>}
            {msg.type === MESSAGE_TYPES.IMAGE && (
              <img src={msg.media.url} alt="" className="max-w-full rounded-lg cursor-pointer" onClick={() => window.open(msg.media.url)} />
            )}
            {msg.type === MESSAGE_TYPES.VIDEO && (
              <video src={msg.media.url} controls className="max-w-full rounded-lg" />
            )}
            {msg.type === MESSAGE_TYPES.FILE && (
              <a href={msg.media.url} target="_blank" rel="noopener noreferrer" className="flex items-center text-blue-600 underline">
                <File size={16} className="mr-1" /> {msg.media.name}
              </a>
            )}
            {msg.type === MESSAGE_TYPES.VOICE && (
              <audio src={msg.media.url} controls className="h-8" />
            )}
            {msg.type === MESSAGE_TYPES.LOCATION && (
              <a href={`https://www.google.com/maps?q=${msg.location.lat},${msg.location.lng}`} target="_blank" rel="noopener noreferrer" className="flex items-center text-blue-600">
                <MapPin size={16} className="mr-1" /> {msg.content}
              </a>
            )}
            {msg.type === MESSAGE_TYPES.CONTACT && (
              <div>
                <p className="font-bold">{msg.contact.name}</p>
                {msg.contact.phoneNumbers?.map((p, i) => <p key={i}>{p.number}</p>)}
              </div>
            )}
            {msg.type === MESSAGE_TYPES.POLL && (
              <PollMessage poll={msg.poll} messageId={msg.id} conversationId={conversationId} />
            )}
            {msg.type === MESSAGE_TYPES.GIFT && (
            ...
            {msg.reactions && Object.keys(msg.reactions).length > 0 && (
              <div className="absolute -bottom-3 right-2 flex bg-white dark:bg-gray-900 rounded-full px-2 py-1 shadow text-sm space-x-1">
                {Object.entries(msg.reactions).map(([uid, reaction]) => (
            ...
            <div className="flex items-center justify-end mt-1 space-x-1">
              ...
              {isOwn && (
                msg.status === 'sending' ? <span className="text-xs">⏳</span> :
                msg.status === 'sent' ? <Check size={12} /> :
                msg.status === 'delivered' ? <CheckCheck size={12} /> :
                msg.status === 'pending' ? <span className="text-xs">⌛</span> : null
              )}
            </div>
          ...
          <div className="flex mt-1 space-x-2 text-xs text-gray-500">
            {isOwn && (
              <>
                <button onClick={() => setEditMessage(msg)} className="hover:text-blue-600">Edit</button>
                <button onClick={() => deleteMessage(msg.id, false)} className="hover:text-red-600">Delete for me</button>
                <button onClick={() => deleteMessage(msg.id, true)} className="hover:text-red-600">Delete for everyone</button>
              </>
            )}
            {!isOwn && (
              <>
                <button onClick={() => setReplyTo(msg)} className="hover:text-blue-600">Reply</button>
                <button onClick={() => reportMessage(msg.id)} className="hover:text-red-600">Report</button>
              </>
            )}
          </div>
        </div>
  ...
  const PollMessage = ({ poll, messageId, conversationId }) => {
    const [voted, setVoted] = useState(false);
    const vote = async (index) => {
      if (voted && !poll.isMultiple) return;
      try {
        await messagingService.voteOnPoll(conversationId, messageId, user.uid, index, poll.isMultiple);
        setVoted(true);
      } catch (err) {
        toast.error(err.message);
      }
