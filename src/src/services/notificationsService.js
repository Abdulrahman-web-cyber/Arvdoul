// src/services/notificationsService.js - ARVDOUL ULTIMATE NOTIFICATIONS V29
...
// ✅ Integrated with userService for rich profiles • Lazy imports • Profile caching
// ✅ Atomic fatigue counters • Configurable sponsored frequency • Enhanced offline flush
// ✅ User‑controlled sound/vibration • Rich media support • Silent push • Analytics export
// ✅ Token cleanup on logout • Notification priority updates • Batch mark by type
...
// ----------------------------------------------------------------------
//  CONFIGURATION – Single source of truth (enhanced)
// ----------------------------------------------------------------------
const NOTIFICATIONS_CONFIG = {
  ...
  PERFORMANCE: {
    ...
    DEBOUNCE_FLUSH_MS: 5000,                     // wait 5 seconds after last offline event before flushing
    USER_PROFILE_CACHE_TTL: 300000               // cache user profiles for 5 minutes
  },
  ...
  MONETIZATION: {
    ...
    AD_INTEGRATION: {
      ...
      REVENUE_SHARE: 0.3,
      ADS_PER_PAGE: 3,
      AD_FREQUENCY: 5                         // insert sponsored ad every N notifications
    },
    ENGAGEMENT_REWARDS: {
  ...
  //    - userId ASC, createdAt DESC (enable TTL on expiresAt)
  // 3. Collection 'user_preferences': default single-field indexes.
...
class UltimateNotificationsService {
  constructor() {
    ...
    this.userProfileCache = new Map();          // cache for user profiles (id -> { name, email, expires })
  ...
  async ensureInitialized() {
    ...
    this.initPromise = (async () => {
      try {
        ...
        // Auth state listener (clean up token refresh on logout)
        ...
        this.sendEmailCF = httpsCallable(functions, 'sendEmailNotification');

        // Network event listener with debounce to flush offline queue
        this.onlineListener = () => {
          console.log('[Notifications] Network online – scheduling offline queue flush');
          ...
          }, NOTIFICATIONS_CONFIG.PERFORMANCE.DEBOUNCE_FLUSH_MS);
        };
  ...
  async _getUserProfile(userId) {
    ...
    try {
      ...
      if (profile) {
        const cacheEntry = {
          profile: { displayName: profile.displayName, email: profile.email, photoURL: profile.photoURL },
          expires: Date.now() + NOTIFICATIONS_CONFIG.PERFORMANCE.USER_PROFILE_CACHE_TTL
        };
        this.userProfileCache.set(userId, cacheEntry);
        return cacheEntry.profile;
      }
    } catch (err) {
      console.warn(`[Notifications] Failed to fetch user profile for ${userId}:`, err);
    }
    return null;
  }
    ...
    try {
      ...
      if (includeSponsored && NOTIFICATIONS_CONFIG.MONETIZATION.AD_INTEGRATION.SPONSORED_NOTIFICATIONS) {
        ...
        if (!prefs.blockedTypes || !prefs.blockedTypes.includes(NOTIFICATIONS_CONFIG.TYPES.SPONSORED)) {
          ...
          if (sponsored.length) {
            notifications = this._interleaveSponsored(notifications, sponsored);
          }
        }
      }
  ...
  async getNotificationById(notificationId) {
    await this.ensureInitialized();
    try {
      const docRef = this.fs.doc(this.firestore, 'notifications', notificationId);
      const snap = await this.fs.getDoc(docRef);
      if (!snap.exists()) return null;
      return { id: snap.id, ...snap.data() };
    } catch (error) {
      throw enhanceError(error, 'Failed to fetch notification');
    }
  }
  ...
  async getUnreadCount(userId) {
    ...
    try {
      ...
      return snap.data().unreadNotifications || 0;
    } catch (error) {
      throw enhanceError(error, 'Failed to fetch unread count');
    }
  }
  ...
  async markNotificationsAsReadByType(userId, type, options = {}) {
    await this.ensureInitialized();
    const { awardCoins = false } = options;
    ...
    try {
      ...
      // Safely update the user's unread counter using a transaction
      const userRef = this.fs.doc(this.firestore, 'users', userId);
      await this.fs.runTransaction(this.firestore, async (transaction) => {
        const userSnap = await transaction.get(userRef);
        if (userSnap.exists()) {
          let newUnread = (userSnap.data().unreadNotifications || 0) - snapshot.size;
          if (newUnread < 0) newUnread = 0;
          transaction.update(userRef, { unreadNotifications: newUnread });
        }
      });

      this._trackEvent('notifications_read_by_type', { userId, type, count: snapshot.size });
      return { success: true, count: snapshot.size };
    } catch (error) {
      this.metrics.errors++;
      throw enhanceError(error, 'Failed to mark notifications by type as read');
    }
  }
  ...
  async clearAllNotifications(userId) {
    ...
    try {
      ...
      this._trackEvent('notifications_cleared', { userId, count: snapshot.size });
      return { success: true, count: snapshot.size };
    } catch (error) {
      this.metrics.errors++;
      throw enhanceError(error, 'Failed to clear notifications');
    }
  }
  ...
  async updateNotificationPriority(notificationId, priority) {
    await this.ensureInitialized();
    if (!['low', 'normal', 'high'].includes(priority)) throw new Error('Invalid priority');
    try {
      const notifRef = this.fs.doc(this.firestore, 'notifications', notificationId);
      await this.fs.updateDoc(notifRef, { priority });
      return { success: true };
    } catch (error) {
      throw enhanceError(error, 'Failed to update priority');
    }
  }

  async resendFailedNotification(notificationId) {
    await this.ensureInitialized();
    try {
      const notif = await this.getNotificationById(notificationId);
      if (!notif) throw new Error('Notification not found');
      // Attempt to resend through same channels
      await this._deliverNotification(notificationId, notif, {});
      return { success: true };
    } catch (error) {
      throw enhanceError(error, 'Failed to resend notification');
    }
  }
  ...
  _setupPushMessageListener() {
    ...
    this.messagingMethods.onMessage(this.messaging, (payload) => {
      ...
      if (payload.notification && !payload.data?.silent) {
  ...
  async _buildNotificationObject(notificationData, options) {
    ...
    // Fetch missing sender/recipient names from userService
    if (!notification.senderName && notification.senderId) {
      const profile = await this._getUserProfile(notification.senderId);
      if (profile && profile.displayName) notification.senderName = profile.displayName;
    }
    if (!notification.recipientName && notification.recipientId) {
      const profile = await this._getUserProfile(notification.recipientId);
      if (profile && profile.displayName) notification.recipientName = profile.displayName;
    }
  ...
  async _deliverNotification(notificationId, notification, options) {
    ...
    if (NOTIFICATIONS_CONFIG.CHANNELS.IN_APP.ENABLED && !notification.silent) {
      results.inApp = true;
    }
    if (NOTIFICATIONS_CONFIG.CHANNELS.PUSH.ENABLED && notification.push !== false && !notification.silent) {
  ...
  async _sendEmailNotification(notificationId, notification) {
    try {
      ...
      if (!recipientEmail && notification.recipientId) {
        const profile = await this._getUserProfile(notification.recipientId);
        if (profile && profile.email) recipientEmail = profile.email;
      }
  ...
  _showNativeNotification(notif) {
    ...
    const options = { body: notif.message, data: { url: notif.actionUrl } };
    if (notif.imageUrl) options.icon = notif.imageUrl;
  ...
  _interleaveSponsored(notifications, sponsored) {
    const frequency = NOTIFICATIONS_CONFIG.MONETIZATION.AD_INTEGRATION.AD_FREQUENCY;
    const maxAds = NOTIFICATIONS_CONFIG.MONETIZATION.AD_INTEGRATION.ADS_PER_PAGE;
    const result = [];
    let adIndex = 0;
    for (let i = 0; i < notifications.length; i++) {
      result.push(notifications[i]);
      if ((i + 1) % frequency === 0 && adIndex < maxAds && adIndex < sponsored.length) {
  ...
  async _tryBatchNotification(notificationData) {
    ...
    const currentCount = batchNotif.data().batchCount || 1;
  ...
  async getNotificationAnalytics(userId, timeFrame = 'week') {
    ...
    try {
      ...
      const opened = events.filter(e => e.eventName === 'notification_open').length;
  ...
  async exportAnalyticsAsCSV(userId, timeFrame = 'week') {
    const data = await this.getNotificationAnalytics(userId, timeFrame);
    const rows = [
      ['Metric', 'Value'],
      ['Sent', data.sent],
    ...
    a.href = url;
    a.download = `notifications_analytics_${userId}_${timeFrame}.csv`;
  ...
  // --------------------------------------------------------------------
  //  USER PREFERENCES (enhanced with sound/vibration)
  // --------------------------------------------------------------------
...
const notificationsService = {
  ...
  getNotificationById: (id) => getNotificationsService().getNotificationById(id),
  getUnreadCount: (uid) => getNotificationsService().getUnreadCount(uid),
  ...
  markNotificationsAsReadByType: (uid, type, opts) => getNotificationsService().markNotificationsAsReadByType(uid, type, opts),
  markAllAsRead: (uid) => getNotificationsService().markAllAsRead(uid),
  clearAllNotifications: (uid) => getNotificationsService().clearAllNotifications(uid),
  deleteNotification: (id, uid, opts) => getNotificationsService().deleteNotification(id, uid, opts),
  updateNotificationPriority: (id, priority) => getNotificationsService().updateNotificationPriority(id, priority),
  resendFailedNotification: (id) => getNotificationsService().resendFailedNotification(id),
  ...
  exportAnalyticsAsCSV: (uid, tf) => getNotificationsService().exportAnalyticsAsCSV(uid, tf),
