...
//   • Integrated with notificationsService for follow/friend‑accepted notifications
//   • Creates default user_preferences document on profile creation
...
class ProfessionalUserService {
  ...
  // Lazy load notifications service to avoid circular deps
  async _getNotificationsService() {
    const module = await import('./notificationsService.js');
    return module.getNotificationsService();
  }
...
<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
  ...
  async createUserProfile(userId, profileData) {
    try {
      ...
      // Create user preferences (required by notifications service)
   ...
   * Also sends a follow notification to the followed user.
   */
  async followUser(followerId, followingId) {
    ...
    // Send follow notification (non‑critical, catch errors)
  ...
  async acceptFriendRequest(requestId, userId) {
    ...
    // We'll need the requester's userId to send notification
    ...
    if (requesterId) {
      try {
        ...
        await notifications.sendNotification({
          ...
          message: `${await this.getUserProfile(userId).then(p => p.displayName || 'Someone')} accepted your friend request`,
          metadata: { friendId: userId }
        });
      } catch (error) {
        console.warn('Failed to send friend-accepted notification:', error);
      }
    }
