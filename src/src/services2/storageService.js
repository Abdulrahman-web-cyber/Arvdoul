...
// 🚀 ULTIMATE UPGRADE: Queue with error recovery • Video thumbnails • Tier limits • Resumable groundwork
...
let getStorageMetadata = null; // renamed to avoid conflict
...
const STORAGE_CONFIG = {
  ...
  // UPGRADE: Tier storage limits (bytes)
  STORAGE_LIMITS: {
    free: 500 * 1024 * 1024,       // 500 MB
    premium: 50 * 1024 * 1024 * 1024, // 50 GB
    creator: 200 * 1024 * 1024 * 1024, // 200 GB
  },

  // UPGRADE: Sharded counters for storage usage
  STORAGE_COUNTERS: {
    SHARD_COUNT: 10,
...
class EnterpriseStorageService {
  constructor() {
    ...
    // UPGRADE: enhanced queue with error recovery
    this.uploadQueue = [];
  ...
  async _getUserStorageUsed(userId) {
    ...
    snapshots.forEach(snap => {
      if (snap.exists) total += snap.data().bytes || 0;
    });
    return total;
  }
  ...
  async uploadFile(file, path, options = {}) {
    ...
    try {
      ...
      if (userId !== 'anonymous') {
        await this._checkStorageLimit(userId, file.size);
      }
      ...
      if (userId !== 'anonymous') {
        await this._incrementStorageUsed(userId, fileToUpload.size);
      }
  ...
  // ==================== UPLOAD QUEUE WITH ERROR RECOVERY ====================
  ...
  async getFileURL(path, options = {}) {
    ...
    try {
      ...
      if (error.code === 'storage/object-not-found') {
        return { success: false, error: 'File not found', url: null, path, operationId };
      }
    ...
    try {
      ...
      if (options.userId) {
        try {
          ...
          if (metadata.success && metadata.metadata?.size) {
      ...
      if (options.userId && fileSize) {
        await this._incrementStorageUsed(options.userId, -fileSize);
      }
      ...
      if (error.code === 'storage/object-not-found') {
        return { success: false, error: 'File not found', path, operationId };
      }
  ...
  async getFileMetadata(path, options = {}) {
    ...
    try {
      ...
      if (error.code === 'storage/object-not-found') {
        return { success: false, error: 'File not found', metadata: null, path, operationId };
      }
