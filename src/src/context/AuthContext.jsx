...
export function AuthProvider({ children }) {
  ...
  const [authError, setAuthError] = useState(null);     // New: holds auth system error
  ...
  const listenerSetUp = useRef(false);   // NEW: prevent multiple listener setups
  ...
  // ========== PERFECT AUTH STATE LISTENER (only once) ==========
  useEffect(() => {
    ...
    const setupAuthListener = async () => {
      try {
        ...
        listenerSetUp.current = true;   // prevent re‑attachment
        ...
        setLoading(false);
        // Still mark as initialized to allow guard to proceed (will show error)
        setAuthInitialized(true);
      }
    };
    ...
    return () => {
      abortController.abort();
      // Note: we do not unset listenerSetUp.current because the effect cleanup
      // will run only when the component unmounts, not on dependency changes.
      // But we want to allow re‑setup if authService changes? We set it only once.
    };
  }, [authService, userService, setCurrentUser, clearUserData]); // Removed authInitialized from deps
  ...
  const contextValue = {
    ...
    loading,
    error,
    authError,        // New: auth system error
    isSignupInProgress,
    authInitialized,
    securityChecks,
    authService,
